...
def center_first():
    ...
    #pos[:, [0, 2]] -= np.mean(pos[:, [0, 2]], axis=0, keepdims=True)
    #print(pos[0, [0, 2]])
    #print(pos[0, [0, 2]]+[0.45, 0])
    #print(pos)
    pos[:, [0, 2]] -= pos[0, [0, 2]]+[0.25, 0]
    #print(pos)
    pyflex.set_positions(pos.flatten())
    pyflex.step()

def random_pick_and_place2(pick_num=10, pick_scale=0.01):
    """ Random pick a particle up and the drop it for pick_num times"""
    curr_pos = pyflex.get_positions().reshape(-1, 4)
    num_particles = curr_pos.shape[0]
    for i in range(pick_num):
        pick_id = np.random.randint(num_particles / 2, num_particles)  ### modified:
        pick_dir = np.random.random(3) * 2 - 1
        pick_dir[1] = (pick_dir[1] + 1)
        pick_dir *= pick_scale
        original_inv_mass = curr_pos[pick_id, 3]
